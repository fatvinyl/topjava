...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    //преобразуем результат запроса в объект типа Meal
    ...
    private SimpleJdbcInsert insertMeal;//объект для добавления значений в таблицу. Нужно предоставить имя таблицы и Map(имя колонки, значение)
    ...
    public List<Meal> getAll(int userId) {
        return jdbcTemplate.query("SELECT * FROM meals WHERE user_id=? ORDER BY date_time DESC", ROW_MAPPER_MEAL, userId);
    }
    ...
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        return jdbcTemplate.query("SELECT * FROM meals WHERE user_id=? AND date_time BETWEEN ? AND ? ORDER BY date_time DESC",
                                    ROW_MAPPER_MEAL, userId, startDate, endDate);
    }
}
